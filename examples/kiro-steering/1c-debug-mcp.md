...
await mcp_1c_debug_set_breakpoints({
  ...
  lines: [42, 100],
  objectID: "4eee25b1-2da6-459b-953b-4c8d519c9bce"  // Опционально, но рекомендуется
});
```
...
### objectID для надёжности

Рекомендуется указывать `objectID` для надёжного сопоставления точек с модулями.
Получить: выгрузить конфигурацию в файлы → открыть XML модуля → атрибут `uuid`.
