...
2. Найти цель с типом `Server` (серверная база) или `ServerEmulation` (файловая база)
...
| Тип | Описание |
|---|---|
| `ManagedClient` | Тонкий клиент (управляемое приложение, `&НаКлиенте`) |
| `Server` | Серверный контекст на сервере 1С (`&НаСервере`, серверная база) |
| `ServerEmulation` | Серверный контекст в файловой базе (эмуляция сервера) |
| `BackgroundJob` | Фоновое задание и регламентные задачи |
